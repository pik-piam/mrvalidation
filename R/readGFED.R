...
readGFED <- function() {
	...
	# Correct emissions units. GFED reports NOx as NO, and MAgPIE outputs NO2.
	emissions_equivalency_table <- data.frame(species = c("CH4", "N2O", "NOx", "NH3"),
											  conversion_factor  = c(1, 1, 23/15, 1))

	d <- inner_join(d, emissions_equivalency_table, by = "species") %>%
		mutate(emission_val = map2(.x = .data$emission_val,
								   .y = .data$conversion_factor,
								   .f = ~ .x * .y)) %>%
		mutate(species = ifelse(.data$species == "NOx",
								"NO2",
								.data$species))
