...
#' @param detail Default is \code{TRU}. If \code{FALSE}, the subcategories of groups are not reported (e.g. "soybean" within "oilcrops")
#'
...
calcValidSelfsuff<-function(datasource="FAO", detail=TRUE){
  ...
  if(datasource=="FAO"){
    ...
    mb2 <- collapseNames(calcOutput("FAOmassbalance",aggregate = F)[,,k_trade][,,"dm"])
    ...
    # infinite values come from zero domestic supply and positive production. By convention, it's set to 99999.
    out[is.na(out)] <- 1 
    out[is.infinite(out)] <- 99999 
